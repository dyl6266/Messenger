...
public class HttpUtils {
	 ...
	 * @return
	 */
	public static String p2no(String s) {
		return s.replaceAll("<p>", "").replaceAll("<p/>", "").replaceAll("<P>", "").replaceAll("<P/>", "").replaceAll("</P>", "").replaceAll("</p>", "").replaceAll("<p", "").replaceAll("<P", "");
	}
	...
	public static String removeEvent(String s) {
	...
	public static String removeNoAllowTag(String contents) {
		try {
			/*
			 * Img List 필요 //IMG src에 허용되지 않는 url 제거 description =
			 * description.replace("&lt;", "<").replace("&gt;", ">"); //&lt;
			 * &gt; 태그 모두 <>로 변환 Pattern pattern =
			 * Pattern.compile("<img[^>]*[>]"); //이미지 태그 모두 찾기 Matcher matcher =
			 * pattern.matcher(description);
			 * 
			 * boolean passFlag; String matcherString; String imot50;
			 * while(matcher.find()) { passFlag = false; matcherString =
			 * matcher.group();
			 * 
			 * //현재 이미지가 DB에 저장되어있는지 찾기 for(ResultSet rs:
			 * imotList.getItemList()){ imot50 =
			 * ((UIMileageRS)rs).getImot().getFile50().toString(); if
			 * (matcherString.indexOf(imot50)>-1) { passFlag = true; break; } }
			 * 
			 * if(!passFlag){ description = description.replace(matcherString,
			 * ""); } }
			 */
			//허용되지 않는 태그 삭제
			String pattern2 = "<(\\/?)(?!\\/####)([^<|>]+)?>";
	 ...
	 * @param URL
	 * 
	 * @return public static List<rssVO> getRSSFeed(String URL) { List<rssVO>
	 * rssList = null; try { URL url_rss = new URL(URL); SyndFeedInput rssInput
	 * = new SyndFeedInput(); SyndFeed feed = rssInput.build(new
	 * XmlReader(url_rss)); if (feed != null) { rssList = new ArrayList<>();
	 * 
	 * @SuppressWarnings("rawtypes") List entries = feed.getEntries(); for (int
	 * index = 0; index < 5; index++) { SyndEntry entry =
	 * (SyndEntry)entries.get(index); rssList.add(new rssVO(entry.getTitle(),
	 * entry.getDescription().getValue(), entry.getLink())); } }
	 * 
	 * } catch(IllegalArgumentException e ) { rssList = null; }
	 * catch(FeedException e) { rssList = null; } catch(Exception e){
	 * //e.printStackTrace(); rssList = null; }
	 * 
	 * return rssList; }
	 */
